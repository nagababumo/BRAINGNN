...
def read_single_data(data_dir, filename, use_gdc=False):
    ...
    try:
        if filename.endswith('.h5'):
            ...
            with open(osp.join(data_dir, filename), 'rb') as f:
                temp = pickle.load(f)
        else:
            raise ValueError("Unsupported file format")
    except Exception as e:
        print(f"Error reading file {osp.join(data_dir, filename)}: {e}")
        return None

    if not isinstance(temp, dict):
        print(f"Error: The loaded data from {filename} is not a dictionary. Attempting to process differently.")
        print(f"Loaded data type: {type(temp)}")
        print(f"Loaded data: {temp}")

        if isinstance(temp, list) and all(isinstance(item, dict) for item in temp):
            temp = temp[0]  # Use the first element if it's a list of dictionaries
        else:
            return None

    if 'pcorr' not in temp or 'corr' not in temp or 'label' not in temp:
        print(f"Error: Missing expected keys in the loaded data from {filename}. Keys found: {list(temp.keys())}")
        return None
